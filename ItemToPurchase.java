...
public class ItemToPurchase {
    ...
    public ItemToPurchase(String name, double price, double quantity) throws InvalidVariableException {
        ...
        if (name == null)
            throw new InvalidVariableException("Name cannot be null");
        if (price < 0)
            throw new InvalidVariableException("Price cannot be negative");
        if (quantity <= 0)
            throw new InvalidVariableException("Quantity must be greater than zero");
        this.itemName = name;
    ...
    // Formats a number: whole numbers show no decimals, others show 2 decimals
    ...
    public String toString() {
        return itemName + " " + formatNumber(itemQuantity) + " @ $" + formatNumber(itemPrice) + " = $"
